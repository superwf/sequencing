...
func GetReactions(req *http.Request)([]map[string]interface{}, int){
  ...
  db := Db.Table("reactions").Select("reactions.id, reactions.primer_id, reactions.remark, orders.sn, orders.remark, orders.status, orders.urgent, orders.is_test, clients.id, clients.name, clients.tel, orders.board_head_id, samples.id, samples.name, samples.vector_id, samples.splice_status, sample_boards.sn, sample_boards.procedure_id, samples.hole, primers.name, reaction_boards.sn, reaction_boards.procedure_id, reactions.hole, prechecks.code_id, reaction_files.code_id, reaction_files.proposal, reaction_files.interpreter_id, vectors.name").Order("reactions.id DESC")
